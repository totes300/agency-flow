...
5.**App shell uses shadcn sidebar-07 block** as the base layout. Do not rebuild the sidebar from scratch its already existinf — adapt the installed sidebar-07 component. Modify nav items, add the header with timer indicator and Cmd+K search trigger, and make it responsive per the PRD.
...
- [x] App runs with Next.js 16 + Turbopack
- [x] Clerk sign-up/sign-in works
- [x] User data syncs to Convex on signup and profile update
- [x] User role is accessible in Convex functions
- [x] Full schema defined (all tables — even if unused until later phases)
- [x] Schema includes server-side timer fields on users (`timerTaskId`, `timerStartedAt`)
- [x] Schema stores time entry dates as `YYYY-MM-DD` strings
- [x] Schema includes `todayOrder` table for per-user task ordering
- [x] Schema includes `workCategories` table for global work categories
- [x] Schema includes cursor-based pagination support on list queries
- [x] Role-based helper functions work (e.g., assert admin)
- [x] App shell renders with responsive sidebar and header
- [x] `Cmd+K` / `Ctrl+K` opens search modal with simple string matching, results grouped by type, selecting navigates
- [x] Admin-only nav items (Daily Summary, Timesheets, Team) hidden from team members; "My Time" visible to all
- [x] Mobile sidebar works as collapsible drawer
